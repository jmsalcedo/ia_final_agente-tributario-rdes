...
\begin{titlepage}
  ...
  \textbf{Demo en vivo:} \href{https://jmsmartinez-ia-final-agente-tributario-rdes.hf.space/}{jmsmartinez-ia-final-agente-tributario-rdes.hf.space} \\
  \textbf{Fecha:} \today
  \end{flushright}
...
\begin{itemize}
  ...
  \item \textbf{Docker + Hugging Face Spaces}: despliegue contenedorizado en cloud gratuito (16~GB de RAM, 2~vCPU).
\end{itemize}
...
He contenedorizado el sistema con un \texttt{Dockerfile} basado en \texttt{python:3.11-slim}. Mi \texttt{docker-compose.yml} expone el puerto 7860 y monta volúmenes para persistir ChromaDB y los logs. He desplegado el contenedor en \textbf{Hugging Face Spaces} con el SDK Docker en plan gratuito (16~GB de RAM, 2~vCPU), donde la sola configuración necesaria es el \emph{secret} \texttt{GROQ\_API\_KEY}. He configurado además un workflow de GitHub Actions (\texttt{.github/workflows/sync-to-hf.yml}) que sincroniza automáticamente cada commit del repositorio principal al Space.

Inicialmente exploré el despliegue en Render, pero su plan gratuito (limitado a 512~MB de RAM) provocaba interrupciones por \emph{Out-Of-Memory} al cargar PyTorch y los embeddings. La migración a Hugging Face Spaces resolvió este problema y además eliminó la latencia de \emph{cold start} característica de los servicios gratuitos genéricos. La aplicación desplegada se encuentra disponible en \url{https://jmsmartinez-ia-final-agente-tributario-rdes.hf.space/}.
...
\midrule
Relevancia $\mathrm{Rel}$        & 0.53 & 0.52 & $-0.01$ \\
Cobertura $\mathrm{Cob}$         & 0.97 & 0.98 & $+0.01$ \\
Fundamentación $\mathrm{Fun}$    & 0.16 & 0.15 & $-0.01$ \\
\midrule
Recompensa $R$ & 0.57 & 0.57 & $\approx 0.00$ \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Análisis comparativo por jurisdicción}\label{subsec:por_pais}

Para evaluar si el sistema mantiene un rendimiento equilibrado entre ambas jurisdicciones (España y República Dominicana), descompongo la distribución de recompensas por país y por estrategia de redacción. La figura~\ref{fig:recompensa_por_pais} presenta este desglose mediante diagramas de caja, donde cada caja agrupa los episodios ejecutados con la política aprendida.

\begin{figure}[H]
\centering
\IfFileExists{figs/recompensa_por_pais.pdf}{%
  \includegraphics[width=0.85\textwidth]{figs/recompensa_por_pais.pdf}%
}{%
  \placeholderfig{Distribucion de recompensa por pais y estrategia}{figs/recompensa\_por\_pais.pdf}%
}
\caption{Distribución de recompensa por país (ES, DO) y estrategia de redacción (política aprendida).}
\label{fig:recompensa_por_pais}
\end{figure}

Del análisis de la figura~\ref{fig:recompensa_por_pais} extraigo tres observaciones relevantes. Primero, las recompensas medias se sitúan en rangos comparables entre ambas jurisdicciones, con una mediana ligeramente superior en República Dominicana ($\approx 0.59$) frente a España ($\approx 0.55$), lo que indica que el sistema \textbf{no presenta sesgo significativo} hacia el corpus español a pesar de que cuantitativamente es mayor. Segundo, la dispersión (rango intercuartílico) es notablemente más amplia en República Dominicana, con valores extremos entre $0.46$ y $0.71$, mientras que España presenta una distribución mucho más compacta entre $0.52$ y $0.59$. Atribuyo esta mayor variabilidad a la menor densidad de documentos indexados para la DGII: cuando la pregunta toca aspectos bien cubiertos en los chunks la recompensa es alta, pero cae bruscamente cuando el corpus es insuficiente. Tercero, observo un sesgo de la política aprendida: para España aparecen las tres estrategias (\texttt{conciso}, \texttt{detallado} y \texttt{paso\_a\_paso}, esta última con muy pocas selecciones), mientras que para República Dominicana sólo se seleccionó \texttt{conciso}; esto es coherente con la fase de exploración inicial del bandit, donde la asignación de estrategias depende del orden de las preguntas evaluadas. Estas observaciones refuerzan la conclusión de que el cuello de botella principal del sistema es la calidad y cobertura del corpus, más que la política de prompting seleccionada por el bandit.

\subsection{Análisis: episodios tempranos vs tardíos}

Para evidenciar el aprendizaje, comparo la recompensa media de los primeros cinco episodios (cuando el bandit aún explora) con la de los cinco últimos (tras 15 actualizaciones). En la figura~\ref{fig:evolucion} se observa que la recompensa por episodio oscila en una banda relativamente estrecha (aproximadamente $[0.46, 0.71]$), con una media móvil de ventana tres que se mantiene cerca de la línea baseline. La media de los primeros cinco episodios es de aproximadamente 0.55 y la de los cinco últimos cercana a 0.59, una mejora modesta de unas $0.04$ unidades en términos absolutos. Observo además en la figura~\ref{fig:bandit} que la política convergió de forma muy marcada hacia la estrategia \texttt{conciso}, seleccionada en 17 de los 20 episodios totales, con una recompensa media de 0.577 frente a 0.534 de \texttt{detallado} (sólo dos selecciones) y 0.529 de \texttt{paso\_a\_paso} (una única selección). Esta concentración es coherente con el comportamiento esperado de $\varepsilon$-greedy cuando una de las acciones se separa pronto del resto: el bandit explota agresivamente y la fase de exploración queda reducida al $\varepsilon = 0.2$.
...
Inspeccionando manualmente las respuestas, observo dos patrones positivos. Primero, el sistema distingue adecuadamente entre los regímenes de ambos países sin contaminación cruzada cuando el corpus está bien etiquetado y el filtro por \texttt{pais} en ChromaDB es efectivo. Segundo, cierra siempre con el aviso legal, cumpliendo el requisito normativo. En cuanto a debilidades, detecto dos problemas relevantes. El primero es la \textbf{baja fundamentación} ($\approx 0.16$ en ambas políticas, como se aprecia en la figura~\ref{fig:comparativa}): aunque el \emph{system prompt} del Redactor exige citar fuentes mediante \texttt{[Fuente~N]}, el LLM no lo hace de forma consistente en todos los párrafos, lo que reduce significativamente la trazabilidad de las afirmaciones. El segundo es que en preguntas muy específicas (cifras concretas de umbrales actualizados) el sistema tiende a parafrasear sin precisión cuantitativa cuando los chunks recuperados no contienen el dato exacto. Estas dos debilidades son las principales oportunidades de mejora para una versión productiva.
...
En este trabajo he demostrado la viabilidad de construir un asistente tributario autónomo basado en arquitectura multiagente, RAG y aprendizaje por refuerzo ligero, con una inversión computacional asequible (modelo Llama 3.1 8B vía API gratuita de Groq, embeddings de 118~MB, vector store sin servidor). En el experimento controlado de 20 episodios, la política $\varepsilon$-greedy que he implementado convergió rápidamente hacia la estrategia \texttt{conciso} (recompensa media de 0.577), ligeramente superior a las otras dos estrategias evaluadas, aunque la mejora respecto a una baseline de estrategia fija resultó modesta. Este resultado, lejos de ser un fracaso, ilustra una propiedad importante del paradigma RAG bien anclado: cuando el corpus y los embeddings son adecuados, la elección concreta del estilo de prompting tiene un impacto secundario respecto a la calidad de la recuperación, lo que se evidencia en cobertura uniformemente alta (\textgreater 0.97) en ambas políticas.

Identifico tres líneas de trabajo futuro como prioritarias, ordenadas por el impacto esperado según las debilidades observadas. \textbf{Primera}, mejorar la fundamentación de las respuestas (actualmente $\approx 0.16$) añadiendo un \textbf{agente autocrítico} (siguiendo el patrón \emph{Constitutional AI}) que revise la respuesta del Redactor y reintroduzca explícitamente los marcadores \texttt{[Fuente~N]} en cada afirmación verificable. \textbf{Segunda}, sustituir el bandit por un \textbf{Contextual Bandit} o un agente de Q-learning con estado para aprender estrategias condicionadas al tipo de pregunta y al país, lo que podría romper la simetría observada entre estrategias en este experimento. \textbf{Tercera}, ampliar el corpus con \textbf{consultas vinculantes de la DGT} (España) y \textbf{normas generales recientes} (DGII), e incorporar un mecanismo de actualización incremental que permita reflejar cambios normativos sin reindexar todo el corpus.

A pesar de las limitaciones, considero que el sistema constituye una base sólida sobre la que iterar: la modularidad de la arquitectura que he diseñado me permite reemplazar cualquier componente (LLM, embedder, política) sin reescribir el resto, y el despliegue automatizado en Hugging Face Spaces facilita la experimentación pública.
...
\begin{itemize}
  ...
  \item \texttt{.github/workflows/sync-to-hf.yml}: sincronización automática del repositorio a Hugging Face Spaces.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Abrir \texttt{http://localhost:7860} en el navegador.
\end{enumerate}

\textbf{Demo en vivo disponible en:} \url{https://jmsmartinez-ia-final-agente-tributario-rdes.hf.space/}
